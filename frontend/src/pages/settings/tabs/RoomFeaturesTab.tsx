...
import { ListOrdered, Plus, Pencil, Search, Sparkles, Trash2, X } from 'lucide-react'
import toast from 'react-hot-toast'
import { useAdminRoomFeatures, useRoomFeatureMutations } from '@/hooks/useAdmin'
...
function featureToForm(item: RoomFeature): RoomFeatureForm {
  return {
    name: item.name,
...
function filterRoomFeatures(
  ...
  return items.filter((item) => {
    ...
    const haystack = [item.name, String(item.sort_order ?? '')].join(' ').toLowerCase()
    return haystack.includes(term)
  })
}

interface RoomFeatureFormModalProps {
  readonly open: boolean
  readonly editing: RoomFeature | null
  readonly form: RoomFeatureForm
  readonly saving: boolean
  readonly onClose: () => void
  readonly onChange: (patch: Partial<RoomFeatureForm>) => void
  readonly onSubmit: (e: SubmitEvent<HTMLFormElement>) => void
}

function RoomFeatureFormModal({
  open,
  editing,
  form,
  saving,
  ...
  onChange,
  onSubmit,
}: RoomFeatureFormModalProps) {
  const title = editing ? 'Editar característica' : 'Nueva característica'
    ...
    <Modal open={open} onClose={onClose} size="md" ariaLabel={title}>
      <form onSubmit={onSubmit} className="flex flex-col min-h-0 max-h-[90vh]">
        ...
        <div className="px-5 py-4 space-y-4 overflow-y-auto">
          <FormField id="rf-name" label="Nombre" required hint="Ej. Aire acondicionado, TV, Internet">
            <input
              id="rf-name"
              type="text"
              required
          ...
          <FormField id="rf-order" label="Orden" hint="Menor = primero en listados y formularios">
            <input
              id="rf-order"
              type="number"
              min={0}
          ...
          >
            <Checkbox
              ...
              label="Disponible al asignar habitaciones"
            />
          </div>
        </div>
...
function RoomFeaturesLoadError() {
  return (
    <div
...
interface RoomFeatureRowProps {
  readonly item: RoomFeature
  readonly onEdit: (item: RoomFeature) => void
  readonly onDelete: (item: RoomFeature) => void
}

function RoomFeatureRow({ item, onEdit, onDelete }: RoomFeatureRowProps) {
  const active = item.is_active ?? true
...
interface RoomFeatureMobileCardProps {
  readonly item: RoomFeature
  readonly onEdit: (item: RoomFeature) => void
  readonly onDelete: (item: RoomFeature) => void
}

function RoomFeatureMobileCard({ item, onEdit, onDelete }: RoomFeatureMobileCardProps) {
  const active = item.is_active ?? true
...
function RoomFeaturesEmptyState({ onCreate }: { readonly onCreate: () => void }) {
  return (
    <div
    ...
    >
      <Sparkles size={32} className="mx-auto mb-3 opacity-60" style={{ color: 'var(--text-muted)' }} />
      <p className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>
        Sin características definidas
      </p>
      <p className="text-xs mt-1 max-w-sm mx-auto" style={{ color: 'var(--text-muted)' }}>
        Crea equipamiento y servicios (aire, TV, WiFi, etc.) para marcarlos en el catálogo de habitaciones.
        Las en uso solo se desactivan al eliminar.
      </p>
      <button
        type="button"
        onClick={onCreate}
...
interface RoomFeaturesListProps {
  readonly items: RoomFeature[]
  readonly onEdit: (item: RoomFeature) => void
  readonly onDelete: (item: RoomFeature) => void
}

function RoomFeaturesList({ items, onEdit, onDelete }: RoomFeaturesListProps) {
  return (
    <>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr style={{ background: 'var(--bg-muted)', color: 'var(--text-muted)' }}>
              <th className="text-left py-2.5 px-4 text-xs font-semibold uppercase tracking-wide">Característica</th>
          ...
          <tbody>
            {items.map((item) => (
              <RoomFeatureRow key={item.id} item={item} onEdit={onEdit} onDelete={onDelete} />
            ))}
          </tbody>
        </table>
      </div>
      ...
      <div className="md:hidden space-y-3">
        {items.map((item) => (
          <RoomFeatureMobileCard key={item.id} item={item} onEdit={onEdit} onDelete={onDelete} />
        ))}
      </div>
    </>
  )
...
export default function RoomFeaturesTab() {
  const { data: items = [], isLoading, isError } = useAdminRoomFeatures()
  const { create, update, remove } = useRoomFeatureMutations()

  const [search, setSearch] = useState('')
  const [statusFilter, setStatusFilter] = useState<StatusFilter>('all')
  const [form, setForm] = useState<RoomFeatureForm>(EMPTY)
  const [editing, setEditing] = useState<RoomFeature | null>(null)
  const [open, setOpen] = useState(false)
  const [deleteTarget, setDeleteTarget] = useState<RoomFeature | null>(null)
  ...
  const filteredItems = useMemo(
    () => filterRoomFeatures(items, search, statusFilter),
  ...
  const openEdit = (item: RoomFeature) => {
    setEditing(item)
    setForm(featureToForm(item))
    setOpen(true)
  }
  ...
  const submit = async (e: SubmitEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.error('No se pudo guardar la característica.')
    }
  }

  const confirmDelete = (item: RoomFeature) => {
    remove.mutate(item.id, {
      onSuccess: () => {
        setDeleteTarget(null)
        toast.success('Característica eliminada o desactivada.')
      },
      onError: () => toast.error('No se pudo eliminar. Puede estar en uso en habitaciones.'),
    })
  }
  ...
  return (
    <>
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs" style={{ color: 'var(--text-muted)' }}>
          {isLoading ? (
            ...
            <>
              ...
              <span className="opacity-40">·</span>
              <span>Catálogo de habitaciones</span>
            </>
          )}
        </div>
        <button
      ...
      {isError && <RoomFeaturesLoadError />}

      {!isLoading && !isError && items.length === 0 && (
        <RoomFeaturesEmptyState onCreate={openCreate} />
      )}
      ...
      {!isLoading && !isError && filteredItems.length > 0 && (
        <RoomFeaturesList items={filteredItems} onEdit={openEdit} onDelete={setDeleteTarget} />
      )}
      ...
      <DeleteConfirmDialog
        ...
        message={
          deleteTarget
            ? `¿Eliminar o desactivar «${deleteTarget.name}»? Si está asignada a habitaciones, solo se desactivará.`
            : ''
        }
