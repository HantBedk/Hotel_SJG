...
set COMPOSE_ALLINONE=docker/compose/docker-compose.allinone.yml
...
set MISSING=0
if not exist "docker\Dockerfile.allinone"              ( echo [FALTA] docker\Dockerfile.allinone & set MISSING=1 )
if not exist "%COMPOSE_ALLINONE%"                      ( echo [FALTA] %COMPOSE_ALLINONE% & set MISSING=1 )
if not exist "docker\entrypoint.allinone.sh"            ( echo [FALTA] docker\entrypoint.allinone.sh & set MISSING=1 )
...
docker compose --project-directory . -f "%COMPOSE_ALLINONE%" down >nul 2>&1
echo        OK
echo.

echo [4/7] Construyendo imagen Docker desde cero (puede tardar 10-15 min)...
docker compose --project-directory . -f "%COMPOSE_ALLINONE%" build --no-cache
if errorlevel 1 (
    echo.
...
docker compose --project-directory . -f "%COMPOSE_ALLINONE%" up -d
if errorlevel 1 (
    echo [AVISO] Fallo al levantar el contenedor local, pero el kit ya esta listo.
    echo         Puedes levantarlo manualmente con:
    echo             docker compose --project-directory . -f %COMPOSE_ALLINONE% up -d
    goto resumen
)
echo        OK
...
echo         Revisa: docker compose --project-directory . -f %COMPOSE_ALLINONE% logs -f
goto resumen
